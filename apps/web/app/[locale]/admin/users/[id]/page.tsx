...
import { authAPI, adminAPI, AdminUser, AdminPackage, AdminRoute, UserResponse } from '@/lib/api'
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
                  ...
                  <>
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        max="300"
                        step="0.1"
                        value={editedUser.max_deviation_km ? kmToMiles(editedUser.max_deviation_km).toFixed(1) : ''}
                        onChange={(e) => setEditedUser({ ...editedUser, max_deviation_km: milesToKm(parseFloat(e.target.value) || 3) })}
                        className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="3"
                      />
                      <span className="text-gray-600 mt-1">mi</span>
                    </div>
                    <p className="text-xs text-gray-500 mt-1">
                      Range: 1-300 mi. Maximum distance from planned route for package matching.
                    </p>
                  </>
                ) : (
                  <>
                    <p className="text-gray-900 mt-1 text-lg font-semibold">
                      {kmToMiles(user.max_deviation_km).toFixed(1)} mi
                    </p>
        ...
        {/* Routes Section - Show for courier/both/admin roles */}
        {user && ['courier', 'both', 'admin'].includes(user.role.toLowerCase()) && (
          <>
            {routes.length > 0 ? (
              <>
              <div className="mt-6 bg-white rounded-lg shadow-md p-6">
                      ...
                      {routes.map((route) => (
                        <tr
                          ...
                          className={`hover:bg-gray-50 cursor-pointer ${
                            selectedRoute?.id === route.id ? 'bg-purple-50 ring-2 ring-purple-500 ring-inset' : ''
                          }`}
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-sm text-gray-900">
                            <div className="max-w-xs truncate" title={route.start_address}>
                              {route.start_address}
                            </div>
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-900">
                            <div className="max-w-xs truncate" title={route.end_address}>
                              {route.end_address}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Click on a route to view it on the map below
                </p>
              </div>
              ...
              <div className="mt-6 bg-white rounded-lg shadow-md p-6">
                ...
                {selectedRoute ? (
                  <>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-purple-600 mt-1">
                            Max deviation: {kmToMiles(selectedRoute.max_deviation_km).toFixed(1)} mi
                            {selectedRoute.trip_date && ` • Trip date: ${new Date(selectedRoute.trip_date).toLocaleDateString()}`}
                          </p>
                        </div>
                        <button
                    ...
                    <RouteMap
                      pickup={{
                        address: selectedRoute.start_address,
                        lat: selectedRoute.start_lat,
                        lng: selectedRoute.start_lng,
                        label: 'Start',
                      }}
                      dropoff={{
                        address: selectedRoute.end_address,
                      ...
                      height={400}
                      showRoute={true}
                      showDeviationRadius={true}
                      deviationKm={selectedRoute.max_deviation_km}
                    />
                    <div className="mt-3 flex items-center justify-between text-sm text-gray-600">
                      <div className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded-full bg-green-500"></span>
                        <span>Start: {selectedRoute.start_address}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>End: {selectedRoute.end_address}</span>
                      </div>
                    </div>
                  </>
                ) : (
                  <div className="flex items-center justify-center h-64 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                    <div className="text-center">
                      ...
                      <p className="text-gray-500">Select a route from the table above to view it on the map</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="mt-6 bg-white rounded-lg shadow-md p-8 text-center">
                ...
                <p className="text-gray-600">This user has no routes yet</p>
              </div>
            )}
          </>
        )}
